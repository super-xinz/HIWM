...
curl -fsS -b "$COOKIE_JAR" "$BASE_URL/api/v1/companion/profile/$USER_ID" >/dev/null
...
printf '%s' "$CHAT_RESPONSE" | python -c 'import json,sys; d=json.load(sys.stdin); assert d["reply"] and d["turn_id"]'
