...
export class Processor {
  ...
  private async _motionDataGroupHandler(avatar_motion_data: IAvatarMotionData) {
    try {
      ...
      console.error('Avatar motion processing failed', {
  ...
  private async _handleAudioConfig(
    ...
    if (audio) {
      ...
      if (lastMotionGroup.player === undefined) {
          ...
          lastMotionGroup.player = new Player(
            {
              ...
              onended: (option) => {
                ...
                console.debug('Avatar playback ended', {
                  endOfBatch: Boolean(innerEndOfBatch),
                })
                if (innerEndOfBatch) {
        ...
        console.debug('Avatar batch metadata received', {
          endOfBatch: Boolean(end_of_batch),
        })
        if (end_of_batch) {
