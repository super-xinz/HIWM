...
<template>
  <div class="chat-input-container">
    <button type="button" class="stop-chat-btn" aria-label="停止对话" @click="onStop" />
    ...
    <div class="chat-input-inner">
      ...
      <template v-if="replying">
        <button type="button" class="interrupt-btn" aria-label="打断回复" @click="onInterrupt">
          <Iconfont :icon="HandStop" :color="'#fff'" />
        </button>
      </template>
      <template v-else>
        <button type="button" class="send-btn" aria-label="发送消息" @click="on_send">
...
.chat-input-container {
  ...
  .chat-input-inner {
    ...
    .interrupt-btn {
      ...
      &:focus-visible {
        outline: 3px solid rgba(97, 92, 237, 0.24);
        outline-offset: 2px;
      }
    }
