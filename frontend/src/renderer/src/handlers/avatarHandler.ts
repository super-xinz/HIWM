...
export class AvatarHandler extends EventEmitter {
  ...
  private _bindEventTypes(): void {
    ...
    this.on(EventTypes.ErrorReceived, () => {
      console.warn('Avatar processing error received')
      this.curState = TYVoiceChatState.Idle
      this.emit(EventTypes.StateChanged, this.curState)
    })
  ...
  private _handleTextMessage(message: string): void {
    ...
    try {
      ...
      console.warn('Failed to parse avatar WebSocket text message')
      return
    }
    ...
    switch (header.name) {
      ...
      case WsProtocol.EchoHumanText: {
        console.debug('Avatar text message received', { role: 'human' })
      ...
      case WsProtocol.EchoAvatarText: {
        console.debug('Avatar text message received', { role: 'avatar' })
    ...
    if (payload) {
      ...
      switch (payload.type) {
        case 'interrupt':
          ...
          break
        case 'stream_cancel':
          console.debug('Avatar stream cancellation received', {
            clientPlayback: payload.stream_type === 'client_playback',
          })
  ...
  private _handleAvatarAudioTextFrame(payload?: EchoAvatarAudioPayload): void {
    ...
    console.debug('Avatar audio metadata received')
  }
