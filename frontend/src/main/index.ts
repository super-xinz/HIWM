...
function createWindow(): void {
  ...
  ipcMain.on('state-changed', (_event, data) => {
    const stateKey = data?.key
    console.debug('Electron state update received', {
      key:
        stateKey === 'toolsVisible' || stateKey === 'inputVisible' || stateKey === 'showChatRecords'
          ? stateKey
          : 'other',
    })
...
app.whenReady().then(() => {
  ...
  electronApp.setAppUserModelId('org.hiwm.interaction-demo')
