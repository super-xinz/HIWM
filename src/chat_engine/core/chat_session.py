...
class ChatSession:
    ...
    def __init__(self, session_context: SessionContext, engine_config: ChatEngineConfigModel):
        ...
        configured_timeouts = [
            value
            for config in (engine_config.handler_configs or {}).values()
            for value in [config.get("request_timeout_seconds")]
            if isinstance(value, (int, float))
            and not isinstance(value, bool)
            and value > 0
        ]
        # Leases are the primary guarantee for slow handlers.  This longer
        # grace period is only a secondary guard for legacy handlers that have
        # not yet acquired a lease explicitly.
        recycle_ttl = max(10.0, max(configured_timeouts, default=0.0) + 10.0)
        self.stream_manager = StreamManager(
            self.signal_manager, recycle_ttl=recycle_ttl
        )
