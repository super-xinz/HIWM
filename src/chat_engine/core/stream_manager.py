...
class StreamRetentionLease:
    """Keep one stream addressable while a handler performs slow work.

    A lease is deliberately separate from a downstream stream reference: the
    handler may not know whether it will produce a normal result or an error
    until the slow operation completes.  Releasing is idempotent so cleanup in
    ``finally`` blocks is safe.
    """

    def __init__(self, storage: "StreamStorage", stream_key: StreamKey):
    ...
    def release(self) -> None:
        ...
        if storage is not None:
            storage.release_stream_lease(self._stream_key)

    def __enter__(self) -> "StreamRetentionLease":
        return self
...
class StreamStorage:
    def __init__(self,
        ...
        self._retention_counts: Dict[StreamKey, int] = {}
    ...
    def _cleanup_recycle(self, force: bool = False):
        ...
        with self._lifecycle_lock:
            ...
            for key in expired_keys:
                if self._retention_counts.get(key, 0) > 0:
                    continue
    ...
    def acquire_stream_lease(
        self, stream_id: ChatStreamIdentity
    ) -> Optional[StreamRetentionLease]:
        """Retain an existing, non-cancelled stream until the lease is released."""

        key = stream_id.key if stream_id is not None else None
        if key is None:
            return None
        with self._lifecycle_lock:
            ...
            if stream is None or stream.status == ChatStreamStatus.CANCELLED:
                return None
            self._retention_counts[key] = self._retention_counts.get(key, 0) + 1
            self._finished_at.pop(key, None)
            return StreamRetentionLease(self, key)

    def release_stream_lease(self, key: StreamKey) -> None:
        """Release one retention count and restart normal recycle accounting."""

        with self._lifecycle_lock:
            count = self._retention_counts.get(key, 0)
            if count <= 1:
                self._retention_counts.pop(key, None)
            else:
                self._retention_counts[key] = count - 1
                return
    ...
    def _check_lifespan_(self, stream: ChatStream):
        with self._lifecycle_lock:
            ...
            # Mark finished streams; actual removal is deferred to recycle cleanup.
            if (
                self._retention_counts.get(key, 0) == 0
                and len(stream.ref_by) == 0
                and stream.status in (
                    ChatStreamStatus.ENDED,
                    ChatStreamStatus.CANCELLED,
                )
            ):
                self._finished_at.setdefault(key, time.monotonic())
            else:
                # Still referenced or leased; ensure it is not in the recycle list.
                self._finished_at.pop(key, None)
            self._cleanup_recycle()
...
class StreamManager:
    ...
    def acquire_stream_lease(
        ...
        """Keep ``stream_id`` addressable across a slow handler operation."""

        return self._stream_storage.acquire_stream_lease(stream_id)
