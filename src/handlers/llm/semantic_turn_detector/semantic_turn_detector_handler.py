...
class SemanticTurnDetectorConfig(HandlerBaseConfigModel):
    ...
    api_key: str = Field(default="", description="API key", repr=False)
    ...
    interrupt_judge_api_key: str = Field(
        default="",
        description="API key for interrupt intent judgment",
        repr=False,
    )
...
只回答"完成"或"未完"，不要有其他内容。"""
    ...
    INTERRUPT_INTENT_PROMPT = """你是一个打断意图分析器。用户正在与数字人对话，并在数字人播报时打断了它。
...
请分析用户的打断意图，并严格按照以下规则回答：

规则 1：如果用户只是想停止当前播报（例如“停”“别说了”“够了”“安静”“等一下”，或英文 "stop"、"enough"、"hold on"、"wait"），回答：pure_interrupt

规则 2：如果用户提出了新问题、新话题或新请求（例如“我想问……”“其实……”“等等，我觉得……”“但是……”“那……呢”“你能……吗”“我需要……”“我想要……”），回答：has_new_topic

规则 3：只能回答 "pure_interrupt" 或 "has_new_topic"，必须与这里的英文标记完全一致；不要输出其他内容、标点或解释。
