...
class TTSContext(HandlerContext):
    ...
    def handle_text_stream(self, data: ChatData, handler: 'HandlerTTS'):
        ...
        if session is None:
            # A metadata-only final packet (for example ``hiwm_error``) must
            # still reach the RTC text channel, but it must not manufacture an
            # AVATAR_AUDIO stream that can never finish.
            if not text:
                return
        ...
        try:
            streamer = self.data_submitter.get_streamer(ChatDataType.AVATAR_AUDIO)
            if text and session.synthesizer is None:
            ...
            if text and session.synthesizer is not None:
                logger.debug("TTS: Submitting text chunk, chars={}", len(text))
                session.synthesizer.streaming_call(text)
            if text_end:
                if session.synthesizer is not None:
                    session.synthesizer.streaming_complete()
                else:
                    # Defensive close for a session whose synthesizer failed to
                    # initialize before a final packet arrived.
                    streamer.cancel_current()
                session.synthesizer = None
                self.api_links.pop(input_stream_key, None)
        except Exception as e:
            logger.error(
                "TTS: Synthesis request failed; error_type={}",
                type(e).__name__,
            )
            session.reset()
            self.api_links.pop(input_stream_key, None)
            output_streamer = self.data_submitter.get_streamer(ChatDataType.AVATAR_AUDIO)
            if output_streamer is not None:
                output_streamer.cancel_current()
    ...
    def destroy_context(self, context: HandlerContext):
        ...
        for session in context.api_links.values():
            try:
                ...
                logger.warning(
                    "Failed to reset BailianTTS session on destroy; error_type={}",
                    type(e).__name__,
                )
...
class CosyvoiceCallBack(ResultCallback):
    ...
    def on_error(self, message) -> None:
        if self.is_cancelled:
            logger.info('TTS: Synthesis error after cancel (expected)')
            return
        logger.error('TTS: Service returned a synthesis error')
        self._submit_end_frame()
