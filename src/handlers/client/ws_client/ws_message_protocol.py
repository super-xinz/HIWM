...
from chat_engine.data_models.client_perception import ClientObservationPayload
from chat_engine.data_models.client_profile import ClientSessionProfilePayload
...
class MessageType(str, Enum):
    ...
    SEND_CLIENT_OBSERVATION = "SendClientObservation"
    SEND_SESSION_PROFILE = "SendSessionProfile"
...
class SendClientObservation(BaseModel):
    """Consent-gated client-derived perception features (never raw media)."""
...
class SendSessionProfile(BaseModel):
    """User-confirmed initial task context; session-local and text-only."""
...
def parse_message(json_data: dict) -> Optional[BaseMessage]:
    ...
    try:
        ...
        message_class_map = {
            ...
            MessageType.SEND_CLIENT_OBSERVATION: SendClientObservation,
            MessageType.SEND_SESSION_PROFILE: SendSessionProfile,
        ...
        # Validation errors can echo rejected payload values. Client perception
        # contains biometric-derived measurements, so log only the error class.
        logger.error(f"解析JSON消息失败 ({type(e).__name__})")
        return None
