...
class BailianASRCallback(RecognitionCallback):
    ...
    def on_event(self, result: RecognitionResult) -> None:
        ...
        if 'text' in sentence:
            if RecognitionResult.is_sentence_end(sentence):
                ...
                logger.debug(
                    "BailianASR sentence finalized, request_id={}, chars={}",
    ...
    def on_error(self, result: RecognitionResult) -> None:
        ...
        logger.error(
            "BailianASR provider error, request_id={}", result.request_id
        )
        self.completed.set()
...
class BailianASRContext(HandlerContext):
    def __init__(self, session_id: str, *, dump_audio: bool = False):
    ...
    def handle_audio_stream(self, data: ChatData, handler: 'HandlerASR'):
        ...
        try:
            ...
            if callback.error_message:
                logger.error("BailianASR recognition failed at provider")
                ...
                if output_streamer is not None:
                    try:
                        ...
                        logger.warning(
                            "BailianASR: Failed to finish output stream on error; "
                            "error_type={}",
                            type(e).__name__,
                        )
                self.api_links.pop(input_stream_key, None)
            ...
            # Length is safe to log and lets operators distinguish a provider
            # completion from a usable transcript without persisting speech.
            logger.info("BailianASR result finalized, chars={}", len(output_text))
            ...
            logger.error(
                "BailianASR recognition exception; error_type={}",
                type(e).__name__,
            )
...
class HandlerASR(HandlerBase, ABC):
    ...
    def create_context(self, session_context, handler_config=None):
        ...
        context = BailianASRContext(
            session_context.session_info.session_id,
            dump_audio=handler_config.dump_audio,
        )
        return context
    ...
    def destroy_context(self, context: HandlerContext):
        ...
        for session in context.api_links.values():
            ...
            try:
                ...
                logger.warning(
                    "Failed to reset BailianASR session on destroy; error_type={}",
                    type(e).__name__,
                )
